...
const AppContent: React.FC = () => {
  ...
  // DEVELOPMENT BYPASS: If database is not connected, allow local login
  const handleLogin = async (email: string, pass: string): Promise<void> => {
     try {
        ...
        if (user) {
         ...
         // DEVELOPMENT BYPASS: If API fails (database not connected), check local credentials
         console.warn('API login failed, trying local bypass...', e);
         
         // Local credentials for development (REMOVE IN PRODUCTION)
         const localAdmins = [
             { email: 'lucasmelo@nexus.com', password: 'lucas102030' },
             { email: 'ray@sexshop.com.br', password: 'ray123' }
         ];
         
         const localUser = localAdmins.find(u => u.email === email && u.password === pass);
         
         if (localUser) {
             console.log('✅ Local bypass login successful!');
             const mockUser = { id: 1, email: localUser.email };
             sessionStorage.setItem('adminLoggedIn', mockUser.email);
